...
urlpatterns=[
    ...
    path("by-user/<int:user_id>/", EventListByUserView.as_view(), name="event-list-by-user"), #id пользователя нужно указывать
    ...
    path("unsubscribe/<int:pk>/", EventUnsubscribeView.as_view(), name="event-unsubscribe"),#id ивента нужно указывать
    path('my-subscriptions/', MySubscriptionListView.as_view(), name='my-subscriptions'),
    path("subscription/<int:pk>/", SubscriptionDetailView.as_view(), name="subscription-detail"), #id подписки нужно указывать
