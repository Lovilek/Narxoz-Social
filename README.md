...
6. **Подключение к бэкенду**  
   - Для разработки и тестирования локально настроен адрес WebSocket в `BuildConfig.BASE_WS_URL`:  
     ...
     ```  
   - При запуске на физическом устройстве замените `10.0.2.2` на IP машины с работающим бэкендом в локальной сети (например, `http://192.168.1.100:8000`).
...
Для эмулятора Android: 10.0.2.2 маппит локальную машину.

Для физического устройства необходимо изменить на IP реального сервера (возможные варианты: "ws://192.168.0.101:8000", если бэкенд запущен локально на 192.168.0.101).
...
При локальной разработке на эмуляторе оставьте 10.0.2.2.
...
Уведомления: пуш-уведомления в мобильном приложении пока не реализованы; планируется через Firebase Cloud Messaging (FCM).
